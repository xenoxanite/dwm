...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, needresize, isterminal, noswallow;
	pid_t pid;
	char scratchkey;
	Client *next;
	Client *snext;
...
resizeclient(Client *c, int x, int y, int w, int h)
{
	...
	if (c->isfloating || c->mon->lt[c->mon->sellt]->arrange == NULL && (&monocle == c->mon->lt[c->mon->sellt]->arrange)) {
	} else {
