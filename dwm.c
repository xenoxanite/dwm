...
struct Client {
  ...
  int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen,
      needresize, isterminal, noswallow;
  pid_t pid;
  char scratchkey;
  Client *next;
  Client *snext;
    ...
    if (text[i] == '^' && !isCode) {
      ...
      while (text[++i] != '^') {
        if (text[i] == 'c') {
          ...
          drw_rect(drw, rx + x - 2 * sp, ry + vertpadbar / 2, rw,
...
void drawbar(Monitor *m) {
  ...
  if ((m == selmon || 1) && showstatus) {
    tw = statusw = m->ww - drawstatusbar(m, bh, stext, stw) - stw - 2 * sp;
  }
  ...
  if ((w = m->ww - tw - stw - x - sp) > bh) {
    if (m->sel && showtitle) {
      ...
      drw_text(drw, x - 2 * sp, vertpadbar / 2, w, bh - vertpadbar, lrpad / 2,
               m->sel->name, 0);
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        ...
        // XRDB_LOAD_COLOR("dwm.color8", seltagbgcolor);
      }
    }
  }
...
void removescratch(const Arg *arg) {
  Client *c = selmon->sel;
  if (!c)
    return;
  c->scratchkey = 0;
...
void resizeclient(Client *c, int x, int y, int w, int h) {
  ...
  // &wc); configure(c); XSync(dpy, False);
...
void togglescratch(const Arg *arg) {
  ...
  Monitor *mon;
  int scratchvisible =
      0; // whether the scratchpads are currently visible or not
  int multimonscratch =
      0; // whether we have scratchpads that are placed on multiple monitors
  int scratchmon = -1;    // the monitor where the scratchpads exist
  int numscratchpads = 0; // count of scratchpads

  /* Looping through monitors and client's twice, the first time to work out
     whether we need to move clients across from one monitor to another or not
   */
  ...
  /* Now for the real deal. The logic should go like:
      ...
      - shown scratchpads will be hidden, unless they are being moved to the
     current monitor
      - the scratchpads will be moved to the current monitor if they all
     reside on the same monitor
      - multiple scratchpads residing on separate monitors will be left in
     place
   */
  for (mon = mons; mon; mon = mon->next) {
    for (c = mon->stack; c; c = next) {
      ...
      /* Record the first found scratchpad client for focus purposes, but
         prioritise the scratchpad on the current monitor if one exists */
      if (!found || (mon == selmon && found->mon != selmon))
        found = c;

      /* If scratchpad clients reside on another monitor and we are moving
         them across then as we are looping through monitors we could be
         moving a client to a monitor that has not been processed yet, hence
         we could be processing a scratchpad twice. To avoid this we detach
         them and add them to a temporary list (monclients) which is to be
         processed later.
       */
      if (!multimonscratch && c->mon != selmon) {
        ...
        detachstack(c);
        c->next = NULL;
        /* Note that we are adding clients at the end of the list, this is to
           preserve the order of clients as they were on the adjacent monitor
           (relevant when tiled) */
        if (last)
          last = last->next = c;
        else
        ...
        XSetWindowBorder(dpy, c->win,
                         scheme[SchemeScratchNorm][ColBorder].pixel);
  ...
  for (c = monclients; c; c = next) {
    ...
    /* Attach scratchpad clients from other monitors at the bottom of the
     * stack
     */
    if (selmon->clients) {
    ...
    /* Center floating scratchpad windows when moved from one monitor to
     * another
     */
    if (c->isfloating) {
      if (c->w > selmon->ww)
      ...
      if (numscratchpads > 1) {
        c->x = c->mon->wx +
               (c->x - mon->wx) * ((double)(abs(c->mon->ww - WIDTH(c))) /
                                   MAX(abs(mon->ww - WIDTH(c)), 1));
        c->y = c->mon->wy +
               (c->y - mon->wy) * ((double)(abs(c->mon->wh - HEIGHT(c))) /
                                   MAX(abs(mon->wh - HEIGHT(c)), 1));
...
void updatesystray(void) {
  ...
  if (!systray) {
    ...
    if (!(systray = (Systray *)calloc(1, sizeof(Systray))))
      die("fatal: could not malloc() %u bytes\n", sizeof(Systray));
    systray->win = XCreateSimpleWindow(dpy, root, x, y, w, bh, 0, 0,
  ...
  for (w = 0, i = systray->icons; i; i = i->next) {
    ...
    wa.background_pixel = scheme[SchemeButtonBar][ColBg].pixel;
  ...
  /* redraw background */
  XSetForeground(dpy, drw->gc, scheme[SchemeButtonBar][ColBg].pixel);
